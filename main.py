...
def test_length(length, typing_mode):
    if typing_mode == 1:
...
def render_text(typing_list):
    x_index = 50
    y_index = 30
    for i in range(len(typing_list)):
        words = font.render(typing_list[i], True, BLACK)
...
def results():
    ...
    total_time = font.render('Time Spent: ' + str(time_spent) + ' seconds', True, BLACK)
    time_rect = total_time.get_rect(center=(500, 350))
    screen.blit(total_time, time_rect)
...
# variables used for handling typing logic
word_list = []
player_typing = []
time_spent = 0
active_typing = ""
...
# booleans used for handling typing logic
active = False
started = False
selected = False
finished = False
